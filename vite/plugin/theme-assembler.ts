...
function assemble(
  ...
  for (const relPath of pageFiles) {
    ...
    if (content.includes("<!--VITE_HEAD_TAGS-->")) {
      if (manifest) {
        const manifestKey = Object.entries(entryNames).find(
          ([, name]) => name === pageName,
        )?.[0];
        if (manifestKey && manifest[manifestKey]) {
          content = content.replace(
            "<!--VITE_HEAD_TAGS-->",
            generateTags(manifest, manifestKey),
          );
        } else {
          logger.warn(
            `[theme-assembler] manifest 中未找到页面入口: ${pageName}，占位符保留`,
          );
        }
      }
  ...
  if (manifest) {
    for (const relPath of pageFiles) {
      ...
      if (existsSync(outFile)) {
        ...
        if (content.includes("VITE_HEAD_TAGS")) {
          logger.error(
            `[theme-assembler] 产物 ${outFileName} 中存在未替换的 VITE_HEAD_TAGS 占位符！`,
          );
        }
      }
    }
  ...
  logger.info(
    `[theme-assembler] 模块: ${moduleFiles.length} 个 PHP → php_modules/`,
  );
