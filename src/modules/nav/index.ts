...
class Nav {
  ...
  constructor() {
    ...
    //监听事件
    eventMitt.on("HEADER_SHOW", this.listenHeader.bind(this));

    //气泡菜单
    this.itemParent.forEach((parent) => {
      const btn: HTMLElement | null = parent.querySelector(
        ".nav-list-item-parent-name",
      );
      const childWrap: HTMLElement | null = parent.querySelector(
        ".nav-list-secondary",
      );
    ...
    for (let [btn, domData] of this.popoverDomData.entries()) {
      ...
      [
        ...
        domData.childWrap.addEventListener(
          eventName as keyof HTMLElementEventMap,
          callback as EventListener,
        );
      });
