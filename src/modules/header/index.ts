...
class Header {
  ...
  constructor() {
    ...
    watch(() => dataStore.scrollY, this.watchCallback, { immediate: true });
  }
  ...
  private watchCallback(val: number, oldVal: number | undefined) {
    if (typeof oldVal === "number" && oldVal > val) {
      ...
      eventMitt.emit("HEADER_SHOW", true);
      return;
    }
    if (val >= this.MAX_SCROLL_Y && this.scrollStatus) {
      ...
      eventMitt.emit("HEADER_SHOW", false);
      return;
    }
  }
}
...
class HeaderNav {
  ...
  private clickOtherCloseMenu(event: MouseEvent) {
    ...
    if (this.mobileMenu.contains(target) || this.navList.contains(target))
      return;
    this.toggleMenu();
  }
}
...
class HeaderSearch {
  ...
  constructor() {
    this.searchWrap = document.querySelector(
      ".header-search-wrap",
    ) as HTMLElement;
    ...
    this.searchHistory = new SearchHistory({
      ...
      closeBlacklist: [
        this.searchWrap,
        document.querySelector(".mobile-search") as HTMLElement,
      ],
  ...
  private inputClick() {
    ...
    if (!dataStore.enableMobileSearch) {
  ...
  private inputBlur() {
    //如果开启了移动端搜索,则不处理open展开
    if (dataStore.enableMobileSearch) return;
...
class MobileSearch {
  ...
  constructor() {
    ...
    //监听
    watch(() => dataStore.windowWidth, this.onResize.bind(this));
  }
  ...
  private onIconClick() {
    ...
    if (!this.init) {
      this.init = true;
      dataStore.enableMobileSearch = true;
  ...
  private onResize(width: number) {
    ...
    if (width > 960) {
      this.init = false;
      dataStore.enableMobileSearch = false;
      //前头插入
...
class LoginDialog {
  ...
  constructor() {
    ...
    const protocolBtn = Array.from(
      document.querySelectorAll(".login-dialog-footer .login-dialog-link"),
    );
